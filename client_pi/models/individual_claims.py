...
class IndividualClaims(models.Model):
    ...
    state = fields.Selection(selection=[
        ...
        default='draft')
    claims_line_id = fields.Many2one('clientpi.indiclaims.line', string='Beneficiary ID')
    claims_line_ids = fields.One2many('clientpi.indiclaims.line', 'claims_id', string='Individual Claims lines',copy=False, readonly=True)
    ...
    def iclaims_cancel(self):
        ...
        #benid = self.env['clientpi.indiclaims.line'].search([('id', '=',benids)], limit=1).benid
        #spcode = self.env['res.partner'].search([('id', '=', self.id)], limit=1).claims_line_id
        invoice_line_ids = []
        # user = self.env['res.users'].search([('id', '=',2)], limit=1).userPI
        # passe = self.env['res.users'].search([('id', '=',2)], limit=1).passe
        # orgid = self.env['res.company'].search([('id', '=',1)], limit=1).orgid
...
class AccountMoveLine(models.Model):
    _name = "clientpi.indiclaims.line"
