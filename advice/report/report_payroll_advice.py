...
class move_advice_report(models.AbstractModel):
    _name = 'report.advice.report_moveadvice'
    ...
    def get_detail(self, line_ids):
        ...
        for l in line_ids:
            ...
            res.update({
                    ...
                    'bysal': l.bysal,
                    'name_bank': l.name_bank,
                    })
            self.total_bysal += l.bysal
            result.append(res)
        result = sorted(result, key=itemgetter('name_bank'))
        return result

    @api.model
    def _get_report_values(self, docids, data=None):
        advice = self.env['account.move.advice'].browse(docids)
        return {
            'doc_ids': docids,
            'doc_model': 'account.move.advice',
            'data': data,
            'docs': advice,
            'time': time,
